...
function comprovante(): InboundMessage {
  return { providerMessageId: "m1", from: "5511988887777", to: "5511999990000", kind: "image", text: null, media: { mimetype: "image/jpeg", base64: "xxx" }, timestamp: new Date() };
}
...
function charge(over: Partial<Charge>): Charge {
  ...
  return {
    ...
    scheduledFor: null, paymentRef: null, paidBy: null, notaSolicitada: null, notaEmitidaEm: null,
...
describe("gate B — o mesmo comprovante não quita duas cobranças", () => {
  ...
  it("pagamento NOVO (outro id) quita a segunda normalmente", async () => {
    ...
    await sm.advance(conv, agentConfig, integration, comprovante());
    // Segundo pagamento de verdade: mesmo valor, ID diferente.
