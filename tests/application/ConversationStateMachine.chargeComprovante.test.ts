...
async function seedVerifiedWithCharge(repos: InMemoryRepositories, opts: { charge: boolean } = { charge: true }) {
  ...
  if (opts.charge) {
    await repos.charges.save({
      ...
      status: "cobrada", calendarEventId: "evt-1", chargedAt: new Date(), paidAt: null, scheduledFor: null, notaSolicitada: null, notaEmitidaEm: null, createdAt: new Date(), updatedAt: new Date(),
    });
  }
  return conv;
}
...
describe("duas cobranças em aberto: o comprovante casa PELO VALOR", () => {
  ...
  async function seedDuasAbertas(repos: InMemoryRepositories) {
    ...
    await repos.charges.save({
      ...
      status: "pendente", calendarEventId: "evt-2", chargedAt: null, paidAt: null, scheduledFor: null, notaSolicitada: null, notaEmitidaEm: null,
